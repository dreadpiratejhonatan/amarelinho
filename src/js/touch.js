...
export class TouchControls {
  ...
  /**
   * Botão E com pointer-events:auto — tap = interagir; arrastar um pouco = olhar.
   * Separado do look global pra não depender de geometry hacks / pointer-events:none.
   */
  _bindInteractButton() {
    ...
    const onMove = (e) => {
      if (this._btnPtrId == null || e.pointerId !== this._btnPtrId || !this._btnLast) return;
      const dx = e.clientX - this._btnLast.x;
      const dy = e.clientY - this._btnLast.y;
      this._btnMoved += Math.hypot(dx, dy);
      // Arrastar no E também olha (depois do slop de tap)
    ...
    const onUp = (e) => {
      if (this._btnPtrId == null || e.pointerId !== this._btnPtrId) return;
      if (this._btnMoved < TAP_SLOP) {
    ...
    // Evita click fantasma / double-fire
